...
from muti_contrast_att import CIA
...
class SGC(nn.Module):
    ...
    def forward(self, x):
        ...
        xpart_f1 = self.cia(att1)
        xpart_f2 = self.cia(att2)
        xpart_f3 = self.cia(att3)
        xpart_f1 = [self.classifier_part(part) for part in xpart_f1]
        xpart_f2 = [self.classifier_part(part) for part in xpart_f2]
        xpart_f3 = [self.classifier_part(part) for part in xpart_f3]

        x_f = [xpart_f1[0], xpart_f2[0], xpart_f3[0]]
        xpart_f1.pop(0)
        xpart_f2.pop(0)
        xpart_f3.pop(0)

        x_part = xpart_f1 + xpart_f2 + xpart_f3

        return xc1, xc2, xc3, x_concat, x_part, x_f
    ...
    def get_params(self):
        ...
        new_layers += list(self.conv_block1.parameters()) + \
                      ...
                      list(self.classifier_part.parameters()) + \
                      list(self.cia.parameters())
