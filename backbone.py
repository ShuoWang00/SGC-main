...
#         fms_boost = fm + alpha*(fm * fms_softmax_boost)             # [10, c, h, w]
#
...
#         fms_suppress = fm * fms_softmax_suppress                    # [10, c, h, w]
#
...
class GFE(nn.Module):
    def __init__(self, in_channel, k):
        ...
        self.k = k
        self.stripconv = nn.Sequential(
            nn.Conv2d(in_channel // k, 1, 1, 1, 0),
            nn.BatchNorm2d(num_features=1),
            nn.ReLU(),
            nn.Sigmoid()
        )
    ...
    def forward(self, fm):
        b, c, w, h = fm.shape
        fms = torch.split(fm, c // self.k, dim=1)
        fms_conv = list(map(self.stripconv, fms))
        fms_softmax = torch.stack(fms_conv, dim=1).sum(dim=1)
        alpha = HyperParams['alpha']
        fms_boost = fm + alpha*(fm * fms_softmax)             # [10, c, h, w]
...
class DenseNet(nn.Module):
    def __init__(self):
        ...
        self.strip1 = GFE(in_channel=768, k=8)
        self.strip2 = GFE(in_channel=2112, k=4)
        self.strip3 = GFE(in_channel=2208, k=2)
...
class ResNet(nn.Module):
    def __init__(self, arch='resnet50'):
        ...
        self.strip1 = GFE(in_channel=512, k=8)
        self.strip2 = GFE(in_channel=1024, k=4)
        self.strip3 = GFE(in_channel=2048, k=2)
